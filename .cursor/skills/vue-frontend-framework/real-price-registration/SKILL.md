...
**不做**：新青安／貸款試算、成交機率、談判話術、自動鑑價模型、扣除車位後單價、主建物佔比篩選、面積 vs 單價圖、可比 Markdown／CSV 匯出、清單欄位排序、門牌全文搜尋、localStorage 持久化可比。
...
14. **樓層區間（選填）**：最低樓層、最高樓層（例如最高填 3＝3 樓以下）；皆空＝不啟用
    - 勾選「樓層條件只套用無電梯案件」（**預設勾選**）：僅對 `電梯 ≠ 有` 的列套用樓層區間；`電梯 = 有`（華廈／大樓等）一律保留
    - 取消勾選：樓層區間套用所有案件
    - 樓層自 `樓別/樓高` 取「／」前段解析（中文或阿拉伯數字層數）；**解析不到則保留該列**

篩選／摘要／圖表都吃篩選後的 `activeTransactions`。`filterOptions` 由目前 active 資料重建（含 `communityNames`）。

`filterTransactions`：若 `selectedCommunityNames.length > 0`，只保留 `communityName` 在集合內者；空白社區名不符任何已選社區。重設篩選時一併清空社區與樓層相關欄位（樓層「僅無電梯」勾選重設回預設勾選）。

### 樓層解析

- 實作於 `parseFloorLevel(floorInfo)`（`realPriceRegistration.service.ts`）
- `三層/五層` → 3；`十一層/十一層` → 11；`全/二層`、空白、無法辨識 → `null`（篩選時保留）
- 只取所在層（／前），不篩總樓高（／後）
